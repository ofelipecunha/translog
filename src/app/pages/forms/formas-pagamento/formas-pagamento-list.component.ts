...
export class FormasPagamentoListComponent implements OnInit {
  ...
  private readonly allRows = signal<FormaPagamentoLinha[]>([]);
  ...
  readonly displayedRows = computed(() => {
    ...
    return this.allRows().filter((row) => {
      ...
      const tipoBusca = (row.tipoRaw ?? '').toLowerCase();
      const tipoLabel = row.tipoLabel.toLowerCase();
      const matchTipo =
        !t || tipoBusca.includes(t) || tipoLabel.includes(t);
  ...
  carregarLista(): void {
    ...
    this.formaPagamentoService
      ...
      .subscribe({
        next: (lista) => {
          this.allRows.set(lista.map(mapearLinhaApi));
          this.pesquisar();
        },
        error: (err: unknown) => {
  ...
  confirmarExcluir(row: FormaPagamentoLinha): void {
    if (
      !confirm(`Excluir a forma de pagamento "${row.nome}"? Esta ação não pode ser desfeita.`)
    ) {
      return;
    }
    ...
    this.formaPagamentoService
      ...
      .subscribe({
        next: () => {
          this.alerts.success(
            'Excluído',
            `A forma de pagamento "${row.nome}" foi excluída com sucesso.`,
          );
          this.carregarLista();
        },
        error: (err: unknown) => {
          const msg = mensagemErroApi(err, 'excluir');
          this.listaErro.set(msg);
  ...
  textoTipo(row: FormaPagamentoLinha): string {
    return row.tipoLabel || etiquetaTipo(row.tipoRaw);
  }
}
